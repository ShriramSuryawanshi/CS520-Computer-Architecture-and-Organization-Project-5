...
/**
 * This is a generic pipeline register that is used to connect pipeline stages
 * together. The register's input is atomically transferred to its output when
...
public class PipelineRegister extends ComponentBase implements IPipeReg {
    ...
    public int getIndexInBefore() {
        return index_in_before;
    }

    public int getIndexInAfter() {
        return index_in_after;
    }
...
//        Logger.out.println("Register " + getName() + " master written with bubble");
     ...
     * register is unable to accept input. This may be a local condition (e.g.
     * the stage is waiting on a resource) or caused indirectly by a later
     * pipeline stage being stalled.
     */
    @Override
    ...
    /**
     * Write to this pipeline register, used by the pipeline stage that has this
     * pipeline register as output.
     *
...
//            output.ins);
    ...
    /**
     * Step forward one clock cycle. When there are no stall conditions, the
     * contents of the master match are moved to the slave latch so that the
     * data can be read by the succeeding pipeline stage.
     */
    @Override
    public void advanceClock() {
        int core_cycle = getCycleNumber();
        if (cycle_number_clock == core_cycle) {
            return;
        }
...
//                getHierarchicalName());
     ...
     * with this pipeline register as its parent for access to things like the
